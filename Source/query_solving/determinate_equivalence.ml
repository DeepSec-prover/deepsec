    ...
    | RStart ->
          ...
          Variable.auto_cleanup_with_reset_notail (fun () ->
            ...
            normalise_configuration conf else_branch original_subst (fun gathering conf_1 ->
              try
                let csys_1 =
                  { csys with
                    original_substitution = gathering.original_substitution;
                    additional_data = { symb_proc with configuration = conf_1 };
                    eq_term = Formula.T.wedge_conjunction gathering.disequations csys.Constraint_system.eq_term
                  }
                in
                let csys_2 = Constraint_sytem.prepare_for_solving_procedure csys_1 in
                csys_set_for_start := csys_2 :: !csys_set_for_start
              with
        ...
        let in_apply_final_test csys_set f_next =
          ...
          then f_next ()
            ...
            if Constraint_system.Set.for_all (fun csys -> csys.Constraint_system.additional_data.origin_process = origin_process) csys_set_1
            then raise (Not_Trace_Equivalent (Constraint_sytem.instantiate csys))
            else
