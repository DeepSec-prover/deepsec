...
let trace_equivalence sem proc1 proc2 =
  ...
  let conf1 = { origin_process = Left; current_process = generic_process_of_process proc1'; trace = [] } in
  let conf2 = { origin_process = Right; current_process = generic_process_of_process proc2'; trace = [] } in
  ...
  let equiv_pbl =
    {
      csys_set = { Constraint_system.set = [csys1;csys2]; Constraint_system.eq_recipe = Formula.R.Top };
      size_frame = 0
    }
  in

  let apply_one_transition = match sem with
    | Private -> apply_one_transition_and_rules_private
    | Classic -> apply_one_transition_and_rules_classic
    | Eavesdrop -> apply_one_transition_and_rules_eavesdrop
  in

  let rec apply_rules equiv_pbl f_next =
    apply_one_transition equiv_pbl apply_rules f_next
  in
