...
module Rewrite_rules : sig
  ...
  (** [get f] returns the rewrite rules associated to the function symbol [f]
      @raise Internal_error when [f] is not a destructor. *)
  val get : symbol -> (protocol_term list * protocol_term) list
  ...
  val display : Display.output -> ?rho:display_renamings option -> symbol -> string
